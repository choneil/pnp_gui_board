...
/**
  * @brief SD MSP init: microSD on SDMMC2 (CN13). Pins per STM32N6570-DK:
  * PC0/PC2/PC3/PC4/PC5 = D2/CLK/CMD/D0/D1 (AF11), PE4 = D3 (AF11).
  */
...
void HAL_DCMIPP_MspInit(DCMIPP_HandleTypeDef* hdcmipp)
{
  ...
  if (hdcmipp->Instance == DCMIPP)
  {
    ...
    gpio.Pin = GPIO_PIN_8; HAL_GPIO_Init(GPIOC, &gpio);
    gpio.Pin = GPIO_PIN_2; HAL_GPIO_Init(GPIOD, &gpio);
    ...
    /* Power on AXISRAM3/4 (camera buffers live in this region) */
    ...
    /* Secure/priv attributes for the DCMIPP master + slave (TBS project) */
...
void HAL_SD_MspDeInit(SD_HandleTypeDef* hsd)
{
  if (hsd->Instance == SDMMC2)
  {
    ...
    HAL_GPIO_DeInit(GPIOC, GPIO_PIN_0 | GPIO_PIN_2 | GPIO_PIN_3 | GPIO_PIN_4 | GPIO_PIN_5);
