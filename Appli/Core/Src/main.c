...
void MX_SDMMC2_SD_Init(void);   /* called by the FileX SD driver glue */
void MX_USB1_OTG_HS_PCD_Init(void);   /* called by the USBX device thread */
/* USER CODE END PFP */
...
/**
  * @brief SDMMC2 initialization (microSD slot CN13). The FSBL clock tree does
  * not route a kernel clock to SDMMC2, so IC4 is configured here from PLL1
  * (800 MHz / 4 = 200 MHz, same kernel rate as ST's Fx_uSD_File_Edit example).
  * Called lazily by the FileX SD driver glue (fx_stm32_sd_init).
  */
void MX_SDMMC2_SD_Init(void)
{
  ...
  {
    /* Card present: capture geometry for the USB MSC LUN. */
    HAL_SD_GetCardInfo(&hsd1, &USBD_SD_CardInfo);
  }
  /* No card is not fatal: FileX retries and USB reports an empty LUN. */
}

/**
  * @brief USB1 OTG HS init (CN6). Called from the USBX device thread.
  */
...
static void MX_GPIO_Init(void)
{
  ...
  {
    /* microSD card-detect input (PN12, low = card present) */
    GPIO_InitTypeDef GPIO_InitStruct = {0};
